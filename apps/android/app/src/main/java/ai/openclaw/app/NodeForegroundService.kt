...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "SolanaOS", text = "Starting...")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title = if (connected) "SolanaOS · Connected" else "SolanaOS"
          val micSuffix =
            if (micEnabled) {
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "SolanaOS connection status"
        setShowBadge(false)
      }
