...
fun TalkOrbOverlay(
  ...
  Column(
        ...
        drawCircle(
          brush =
            Brush.radialGradient(
              colors =
                listOf(
                  seamColor.copy(alpha = 0.96f),
                  mobileAccent.copy(alpha = 0.48f),
                  mobileSurface.copy(alpha = 0.78f),
                ),
              center = center,
    ...
    if (showStatus) {
      Surface(
        color = mobileSurfaceStrong.copy(alpha = 0.92f),
        shape = CircleShape,
        border = androidx.compose.foundation.BorderStroke(1.dp, mobileAccent.copy(alpha = 0.26f)),
      ) {
        Text(
          text = trimmed,
