  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "SolanaOS Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "SolanaOS Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "SolanaOS Active")
      }
