...
class CameraCaptureManager(private val context: Context) {
  ...
  @Volatile private var boundPreviewView: PreviewView? = null
  @Volatile private var boundPreviewFacing: String = "back"
  @Volatile private var boundPreviewDeviceId: String? = null
  @Volatile private var boundPreviewCapture: ImageCapture? = null
  ...
  suspend fun bindPreview(
    ...
    ensureCameraPermission()
    val owner = lifecycleOwner ?: throw IllegalStateException("UNAVAILABLE: camera not ready")
    val normalizedFacing = facing.trim().lowercase().takeIf { it == "front" || it == "back" } ?: "back"
    val provider = context.cameraProvider()
    val shouldRebind =
      boundPreviewView !== previewView ||
        boundPreviewCapture == null ||
        boundPreviewFacing != normalizedFacing ||
        boundPreviewDeviceId != deviceId
    if (!shouldRebind) return@withContext

    val selector = resolveCameraSelector(provider, normalizedFacing, deviceId)
    ...
    boundPreviewFacing = normalizedFacing
    boundPreviewDeviceId = deviceId
    boundPreviewCapture = capture
  }

  suspend fun unbindPreview() =
    withContext(Dispatchers.Main) {
      context.cameraProvider().unbindAll()
      boundPreviewView = null
      boundPreviewCapture = null
      boundPreviewDeviceId = null
      boundPreviewFacing = "back"
    }
  ...
  suspend fun capturePreviewFrame(
    ...
    withContext(Dispatchers.Main) {
      ...
      val capture =
        boundPreviewCapture
          ?: throw IllegalStateException("UNAVAILABLE: live camera preview is not active")
      val (bytes, orientation) = capture.takeJpegWithExif(context.mainExecutor())
      val decoded = BitmapFactory.decodeByteArray(bytes, 0, bytes.size)
        ?: throw IllegalStateException("UNAVAILABLE: failed to decode captured image")
      val rotated = rotateBitmapByExif(decoded, orientation)
      encodeJpegFrame(
        bitmap = rotated,
        quality = quality.coerceIn(0.1, 1.0),
        maxWidth = maxWidth.coerceAtLeast(256),
      )
    }
  ...
  private fun encodeJpegFrame(
    ...
    val scaled =
      if (maxWidth > 0 && bitmap.width > maxWidth) {
        val height =
          (bitmap.height.toDouble() * (maxWidth.toDouble() / bitmap.width.toDouble()))
            .roundToInt()
            .coerceAtLeast(1)
        bitmap.scale(maxWidth, height)
      } else {
        bitmap
      }
    ...
    try {
      ...
      val result =
        JpegSizeLimiter.compressToLimit(
          ...
          encode = { width, height, jpegQuality ->
            val candidate =
            ...
            if (!candidate.compress(Bitmap.CompressFormat.JPEG, jpegQuality, out)) {
              if (candidate !== scaled) candidate.recycle()
  ...
  private fun jpegFramePayload(frame: JpegFrame): Payload =
    Payload(
      """{"format":"jpg","base64":"${frame.base64}","width":${frame.width},"height":${frame.height}}""",
    )
