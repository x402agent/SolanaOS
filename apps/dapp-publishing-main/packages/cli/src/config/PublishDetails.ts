...
const getAndroidDetails = async (
  aaptDir: string,
  apkPath: string,
  developerOverridenLocales: string[] | null
  ...
  try {
    ...
    if (
      (developerOverridenLocales == null && localeArray.length >= 60) ||
      ((developerOverridenLocales?.length ?? 0) >= 60)
    ) {
      showMessage(
...
export const writeToPublishDetails = async ({ app, release, lastSubmittedVersionOnChain, lastUpdatedVersionOnStore }: SaveToConfigArgs) => {
  ...
  const newConfig: PublishDetails = {
    ...
    release: {
      ...
      android_details: {
        android_package: release?.android_details?.android_package ?? currentConfig.release.android_details?.android_package ?? "",
        ...
        permissions: release?.android_details?.permissions ?? currentConfig.release.android_details?.permissions ?? [],
