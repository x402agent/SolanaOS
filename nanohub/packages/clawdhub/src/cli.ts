...
const program = new Command()
  ...
  .description(
    `${styleTitle(`SolanaOS Hub CLI ${getCliBuildLabel()}`)}\n${styleEnvBlock(
      'install, update, search, and publish agent skills for SolanaOS.',
    )}`,
  )
...
async function hasNanoHubMarker(workdir: string) {
  const nanohubLockfile = join(workdir, '.nanohub', 'lock.json')
  if (await pathExists(nanohubLockfile)) return true
  const nanohubMarkerDir = join(workdir, '.nanohub')
  if (await pathExists(nanohubMarkerDir)) return true
