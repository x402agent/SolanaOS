...
describe("agent state local", () => {
  const originalStateDir = process.env.SOLANAOS_STATE_DIR;

  afterEach(() => {
    if (originalStateDir === undefined) delete process.env.SOLANAOS_STATE_DIR;
    else process.env.SOLANAOS_STATE_DIR = originalStateDir;
  });
