...
const resolveAndValidateLocalMediaPath = (raw: string): { resolved: string; mime: string } => {
  ...
  const allowedRoot = path.join(os.homedir(), ".solanaos");
...
const validateRemoteMediaPath = (raw: string): { remotePath: string; mime: string } => {
  ...
  // Remote side enforces ~/.solanaos; this guard lets Studio on macOS request
  // /home/ubuntu/.solanaos/... without tripping local homedir checks.
  const normalized = trimmed.replaceAll("\\\\", "/");
  const inSolanaos =
    normalized === "~/.solanaos" ||
    normalized.startsWith("~/.solanaos/") ||
    normalized.includes("/.solanaos/");
  if (!inSolanaos) {
    throw new Error("Refusing to read remote media outside ~/.solanaos");
  }
