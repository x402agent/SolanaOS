...
import {
  ...
  type GatewayHelloOk,
} from "./solanaos/GatewayBrowserClient";
import type {
...
import { resolveStudioProxyGatewayUrl, isServerProxyAvailable } from "@/lib/gateway/proxy-url";
...
const doctorFixHint =
  "Run `npx solanaos doctor --fix` on the gateway host (or `pnpm solanaos doctor --fix` in a source checkout).";
...
export const useGatewayConnection = (
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        ...
        // Prefer server-side settings, fall back to browser localStorage
        // (used on Netlify / static deployments where no server proxy exists)
        let nextGatewayUrl = gateway?.url?.trim() ? gateway.url : "";
        let nextToken =
          gateway && "token" in gateway && typeof gateway.token === "string"
            ? gateway.token
            : "";
        if (!nextGatewayUrl) {
          const browser = loadBrowserGatewaySettings();
          if (browser.gatewayUrl) {
            nextGatewayUrl = browser.gatewayUrl;
            nextToken = nextToken || browser.token;
          }
        }
        if (!nextGatewayUrl) {
          nextGatewayUrl = DEFAULT_UPSTREAM_GATEWAY_URL;
        }
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      // When the custom server proxy is available (local dev / self-hosted),
      // route through /api/gateway/ws. On Netlify / static deployments,
      // connect directly to the user's upstream gateway URL.
      const useProxy = isServerProxyAvailable();
      const connectUrl = useProxy ? resolveStudioProxyGatewayUrl() : gatewayUrl;
      await client.connect({
        gatewayUrl: connectUrl,
        token,
        authScopeKey: gatewayUrl,
        clientName: "solanaos-control-ui",
      });
      if (useProxy) {
