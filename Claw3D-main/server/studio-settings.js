...
const NEW_STATE_DIRNAME = ".solanaos";
...
const readSolanaosGatewayDefaults = (env = process.env) => {
  try {
    const stateDir = resolveStateDir(env);
    const configPath = path.join(stateDir, SOLANAOS_CONFIG_FILENAME);
...
const loadUpstreamGatewaySettings = (env = process.env) => {
  ...
  if (!token) {
    const defaults = readSolanaosGatewayDefaults(env);
    if (defaults) {
      return {
