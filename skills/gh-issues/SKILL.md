...
user-invocable: true
metadata:
  { "nanosolana": { "requires": { "bins": ["curl", "git", "gh"] }, "primaryEnv": "GH_TOKEN" } }
---
...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by NanoSolana. Pass it as a Bearer token in all API calls:
...
```
cat ~/.nanosolana/nanosolana.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```

If still empty, check `/data/.tamagobot/nanosolana.json`:

```
cat /data/.tamagobot/nanosolana.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the NanoSolana dashboard or in ~/.nanosolana/nanosolana.json under skills.entries.gh-issues."
...
7. **Check claim-based in-progress tracking:**
   ...
   ```
   CLAIMS_FILE="/data/.tamagobot/gh-issues-claims.json"
   if [ ! -f "$CLAIMS_FILE" ]; then
     mkdir -p /data/.tamagobot
     echo '{}' > "$CLAIMS_FILE"
   fi
   ```
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  ```
  CURSOR_FILE="/data/.tamagobot/gh-issues-cursor-{SOURCE_REPO_SLUG}.json"
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., nanosolana-nanosolana)
  ```
...
GH_TOKEN=$(cat ~/.nanosolana/nanosolana.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.tamagobot/nanosolana.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.tamagobot/nanosolana.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export GH_TOKEN=$(cat ~/.nanosolana/nanosolana.json 2>/dev/null | node -e "const fs=require('fs');const d=JSON.parse(fs.readFileSync(0,'utf8'));console.log(d.skills?.entries?.['gh-issues']?.apiKey||'')")
