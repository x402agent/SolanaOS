...
echo ""
echo -e "${PURPLE}╔═══════════════════════════════════════════════════════════════════╗${RESET}"
echo -e "${PURPLE}║${RESET}  ${GREEN}✓ SolanaOS running${RESET}                                                ${PURPLE}║${RESET}"
echo -e "${PURPLE}╚═══════════════════════════════════════════════════════════════════╝${RESET}"
echo ""
echo -e "  ${DIM}Gateway:${RESET}         http://localhost:${GATEWAY_PORT:-18790}"
echo -e "  ${DIM}Gateway health:${RESET}  http://localhost:${GATEWAY_PORT:-18790}/health"
echo -e "  ${DIM}Control UI:${RESET}      http://localhost:${CONTROL_PORT:-7777}"
echo -e "  ${DIM}MCP server:${RESET}      http://localhost:$MCP_PORT/mcp"
echo -e "  ${DIM}solana-claude:${RESET}   http://localhost:3000/mcp"
echo -e "  ${DIM}Wallet API:${RESET}      http://localhost:$WALLET_PORT/v1/health"
echo ""
echo -e "  ${PURPLE}Claude Desktop config:${RESET}"
echo -e '    {'
echo -e "      ${GREEN}\"solanaos\"${RESET}: { \"command\": \"node\", \"args\": [\"$SCRIPT_DIR/mcp-server/dist/index.js\"] },"
echo -e "      ${GREEN}\"solana-claude\"${RESET}: { \"command\": \"node\", \"args\": [\"$SCRIPT_DIR/solana-claude/mcp-server/dist/index.js\"] }"
echo -e '    }'
echo ""
echo -e "  ${PURPLE}Key docs:${RESET}"
echo -e "  ${DIM}DAEMON.md${RESET}   — full architecture, all 55 packages, OODA loop"
echo -e "  ${DIM}SOUL.md${RESET}     — agent identity + trading philosophy (system prompt)"
echo -e "  ${DIM}SKILL.md${RESET}    — agent skill file (give to any AI to operate SolanaOS)"
echo -e "  ${DIM}STRATEGY.md${RESET} — multi-venue trading playbook"
echo -e "  ${DIM}TRADE.md${RESET}    — pump.fun trading tiers + Jupiter execution"
echo -e "  ${DIM}PUMP.md${RESET}     — scanner report format + meta analysis"
echo ""
echo -e "  ${DIM}Logs:    .logs/  (gateway.log, daemon.log, mcp-root.log)${RESET}"
echo -e "  ${DIM}Stop:    bash start.sh --stop${RESET}"
echo -e "  ${DIM}Status:  bash start.sh --status${RESET}"
echo ""
