...
import { SolanaOSApp } from "./app";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = SolanaOSApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("solanaos-app") as SolanaOSApp;
...
beforeEach(() => {
  SolanaOSApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__SOLANAOS_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  SolanaOSApp.prototype.connect = originalConnect;
  window.__SOLANAOS_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/solanaos/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/solanaos");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/solanaos/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SOLANAOS_CONTROL_UI_BASE_PATH__ = "/solanaos";
    const app = mountApp("/solanaos/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/solanaos");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/solanaos/sessions");
  });
